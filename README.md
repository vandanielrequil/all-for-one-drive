...
- Upload идёт с `--transfers=4` (дефолт rclone). Не разгоняйте параллелизм: у Google Drive / Box есть лимиты на число API-запросов в секунду, а годовая заливка тысяч мелких фото как раз бьёт по rate limit.
