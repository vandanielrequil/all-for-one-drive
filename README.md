...
bin/
  ...
  convert.exe                   ← только конвертация фото/видео
  ...
  convert.log                   ← лог отдельной конвертации
...
```jsonc
{
  ...
  "maxUsageMB": 9900,           // опционально: потолок суммарного объёма (rclone size)
  "maxFileSizeMB": 0,           // опционально: лимит на один файл (Box free = 250)
  "accounts": [{
    "name": "primary",
...
Файлы больше `maxFileSizeMB` для этого провайдера не грузятся туда — уходят на следующий storage того же эшелона (spillover).
...
```jsonc
"maxFileSizeMB": 250,  // free Box: жёсткий лимит 250 МБ на файл
"oauth": { "tokenFile": "./box-personal.oauth-token" },
"options": {}
```

Крупные 4K-видео (>250 МБ) Box отклонит — они автоматически уйдут на MEGA/Drive/другие цели эшелона.
...
# только конвертация фото/видео → image-output / video-output
.\convert.exe
.\convert.exe -config D:\path\to\all-for-one.config.jsonc
...
`convert.exe` — только image/video converter, без PreCheck, архивов и upload.  
...
- Upload отдаёт файлы rclone пачками (`rclone.batchSize`, по умолчанию `32`); параллелизм внутри пачки — `rclone.transfers` (по умолчанию `4`). Не разгоняйте `transfers`: у Google Drive / Box есть лимиты на число API-запросов в секунду.
- Конвертеры и staging сохраняют **mtime исходника**; rclone копирует с `--metadata`, чтобы в облаке файлы сортировались по дате съёмки/создания, а не по дате запуска скрипта.
...
cmd/convert/                — только конвертация фото/видео
